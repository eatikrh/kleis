...
mod golden_sets_and_logic {
    ...
    fn set_membership() {
        ...
        let set_examples: Vec<&str> = samples
            .iter()
        ...
        assert!(
            !set_examples.is_empty(),
            "Should have set membership examples"
        );
    ...
    fn universal_quantifier() {
        ...
        let samples = kleis::render::collect_samples_for_gallery();
        let logic_examples: Vec<&str> = samples
            .iter()
        ...
        assert!(
            !logic_examples.is_empty(),
            "Should have quantifier examples"
        );
        assert!(
            logic_examples[0].contains(r"\forall"),
            "Should use \\forall symbol"
        );
    }
}
...
mod golden_top5_operations {
    ...
    fn braket_notation() {
        ...
        // Find ket example
        let ket_samples: Vec<&str> = samples
            .iter()
        ...
        // Find bra example
        let bra_samples: Vec<&str> = samples
            .iter()
    ...
    fn commutator_notation() {
        ...
        let comm_samples: Vec<&str> = samples
            .iter()
    ...
    fn multiple_integrals() {
        ...
        let double_int: Vec<&str> = samples
            .iter()
            .filter(|(t, _)| t.contains("Double"))
            .map(|(_, latex)| latex.as_str())
            .collect();

        let triple_int: Vec<&str> = samples
            .iter()
    ...
    fn square_root_notation() {
        ...
        let sqrt_samples: Vec<&str> = samples
            .iter()
    ...
    fn set_theory_complete() {
        ...
        let set_ops: Vec<&String> = samples
            .iter()
            .filter(|(t, _)| {
                t.contains("Set")
                    || t.contains("membership")
                    || t.contains("Subset")
                    || t.contains("quantifier")
            })
            .map(|(t, _)| t)
            .collect();
        ...
        assert!(
            set_ops.len() >= 3,
            "Should have multiple set operations, found: {:?}",
            set_ops
        );
    }
}
...
mod golden_next_batch {
    ...
    fn comparison_operators() {
        ...
        let comparisons: Vec<&String> = samples
            .iter()
            .filter(|(t, _)| {
                t.contains("constraint")
                    || t.contains("Inequality")
                    || t.contains("equal")
                    || t.contains("Approximation")
            })
            .map(|(t, _)| t)
            .collect();
    ...
    fn complex_numbers() {
        ...
        let complex_ops: Vec<&str> = samples
            .iter()
            .filter(|(t, _)| {
                t.contains("Complex") || t.contains("conjugate") || t.contains("Real and imaginary")
            })
            .map(|(_, latex)| latex.as_str())
            .collect();
    ...
    fn trig_functions() {
        ...
        let trig: Vec<&str> = samples
            .iter()
            .filter(|(t, _)| {
                t.contains("Trigonometric")
                    || t.contains("Hyperbolic")
                    || t.contains("Euler formula")
            })
            .map(|(_, latex)| latex.as_str())
            .collect();
    ...
    fn matrix_operations() {
        ...
        let matrix_ops: Vec<&str> = samples
            .iter()
    ...
    fn quantum_mechanics_complete() {
        ...
        let qm_examples: Vec<&String> = samples
            .iter()
            .filter(|(t, _)| {
                t.contains("QM")
                    || t.contains("Hamiltonian")
                    || t.contains("Schrodinger")
                    || t.contains("Trace")
            })
            .map(|(t, _)| t)
            .collect();
        ...
        assert!(
            qm_examples.len() >= 3,
            "Should have complete QM notation support"
        );
    }
}
...
mod golden_batch4_polish {
    ...
    fn number_sets_render_unicode() {
        ...
        let hilbert_examples: Vec<&str> = samples
            .iter()
...
mod golden_batch3_completeness {
    ...
    fn factorial_notation() {
        ...
        let factorial_examples: Vec<&str> = samples
            .iter()
    ...
    fn binomial_coefficients() {
        ...
        let binom_examples: Vec<&str> = samples
            .iter()
    ...
    fn floor_ceiling_functions() {
        ...
        let bracket_examples: Vec<&str> = samples
            .iter()
    ...
    fn pauli_matrices() {
        ...
        assert!(
            pauli.len() >= 2,
            "Should have at least 2 Pauli matrix examples"
        );
    ...
    fn vector_calculus_operators() {
        ...
        let vec_calc: Vec<&String> = samples
            .iter()
            .filter(|(t, _)| {
                t.contains("Divergence")
                    || t.contains("Curl")
                    || t.contains("Laplacian")
                    || t.contains("Maxwell")
            })
            .map(|(t, _)| t)
            .collect();
        ...
        assert!(
            vec_calc.len() >= 3,
            "Should have divergence, curl, and Laplacian"
        );
    }
    ...
    fn complete_function_library() {
        ...
        let functions: Vec<&String> = samples
            .iter()
            .filter(|(t, _)| {
                t.contains("Trigonometric")
                    || t.contains("Hyperbolic")
                    || t.contains("Logarithms")
                    || t.contains("Inverse trig")
...
mod golden_text_mode {
    ...
    fn text_in_piecewise() {
        ...
        let expected_latex =
            r"\begin{cases}x^{2} & \text{if } x \geq 0\\0 & \text{otherwise}\end{cases}";
...
mod golden_ellipsis {
    ...
    fn cdots_notation() {
        ...
        let cdots_examples: Vec<&str> = samples
            .iter()
        ...
        assert!(
            !cdots_examples.is_empty(),
            "Should have horizontal centered ellipsis"
        );
    ...
    fn ldots_notation() {
        ...
        let ldots_examples: Vec<&str> = samples
            .iter()
        ...
        assert!(
            !ldots_examples.is_empty(),
            "Should have horizontal lower ellipsis"
        );
    ...
    fn vdots_notation() {
        ...
        let vdots_examples: Vec<&str> = samples
            .iter()
    ...
    fn ddots_notation() {
        ...
        let ddots_examples: Vec<&str> = samples
            .iter()
    ...
    fn iddots_notation() {
        ...
        let iddots_examples: Vec<&str> = samples
            .iter()
    ...
    fn ellipsis_in_sequences() {
        ...
        let seq_examples: Vec<&str> = samples
            .iter()
        ...
        assert!(
            !seq_examples.is_empty(),
            "Should have sequence examples with ellipsis"
        );
    ...
    fn ellipsis_in_matrices() {
        ...
        let matrix_examples: Vec<&str> = samples
            .iter()
        ...
        assert!(
            !matrix_examples.is_empty(),
            "Should have matrix examples with ellipsis"
        );
    ...
    fn complete_ellipsis_support() {
        ...
        let ellipsis_examples: Vec<&String> = samples
            .iter()
        ...
        assert!(
            ellipsis_examples.len() >= 6,
            "Should have at least 6 ellipsis examples (4 standard types + 2 use cases), found: {}",
            ellipsis_examples.len()
        );
    }
}
...
fn gallery_output_stability() {
    ...
    assert!(
        samples.len() >= 15,
        "Gallery should have at least 15 examples"
    );

    // Verify key examples are present
    let titles: Vec<&str> = samples.iter().map(|(t, _)| t.as_str()).collect();
    assert!(
        titles.iter().any(|t| t.contains("Einstein")),
        "Should have Einstein equations"
    );
    assert!(
        titles.iter().any(|t| t.contains("Maxwell")),
        "Should have Maxwell tensor"
    );
    assert!(
        titles.iter().any(|t| t.contains("zeta")),
        "Should have zeta function"
    );
