...
fn test_dap_stacktrace_has_source() {
    ...
    let mut dap = TcpStream::connect(format!("127.0.0.1:{}", port))
        .expect("Failed to connect to DAP server");
    
    // DAP initialize
    let dap_init = r#"{"seq":1,"type":"request","command":"initialize","arguments":{"adapterID":"kleis"}}"#;
    ...
    let stack_trace = r#"{"seq":4,"type":"request","command":"stackTrace","arguments":{"threadId":1}}"#;
    send_dap_message(&mut dap, stack_trace);
    thread::sleep(Duration::from_millis(300));
    let stack_resp = read_dap_response(&mut dap);
    
    println!("\n=== STACK TRACE RESPONSE ===");
    println!("{}", serde_json::to_string_pretty(&stack_resp).unwrap_or_default());
    ...
    if let Some(frames) = stack_frames {
        for (i, frame) in frames.iter().enumerate() {
            ...
            println!("Frame {}: name={}, path={}, line={}", i, source_name, source_path, line);
            
            // Assert that source is not "unknown"
            assert_ne!(source_name, "unknown", "Source name should not be 'unknown'");
