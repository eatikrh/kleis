...
fn test_reverse_empty_list() {
    ...
    assert!(
        matches!(expr, Expression::List(ref v) if v.is_empty()),
        "reverse([]) should be []: {:?}",
        expr
    );
}
...
fn test_sum_empty_list() {
    ...
    let expr = result.unwrap();
    assert!(
        matches!(expr, Expression::Const(ref s) if s == "0"),
        "sum([]) should be 0: {:?}",
        expr
    );
}
...
fn test_sum_single_element() {
    ...
    let expr = result.unwrap();
    assert!(
        matches!(expr, Expression::Const(ref s) if s == "42"),
        "sum([42]) should be 42: {:?}",
        expr
    );
}
...
fn test_sum_multiple_elements() {
    ...
    let expr = result.unwrap();
    assert!(
        matches!(expr, Expression::Const(ref s) if s == "10"),
...
fn test_product_empty_list() {
    ...
    let expr = result.unwrap();
    assert!(
        matches!(expr, Expression::Const(ref s) if s == "1"),
        "product([]) should be 1: {:?}",
        expr
    );
}
...
fn test_product_single_element() {
    ...
    let expr = result.unwrap();
    assert!(
        matches!(expr, Expression::Const(ref s) if s == "7"),
        "product([7]) should be 7: {:?}",
        expr
    );
}
...
fn test_product_multiple_elements() {
    ...
    let expr = result.unwrap();
    assert!(
        matches!(expr, Expression::Const(ref s) if s == "24"),
        "product([2,3,4]) should be 24: {:?}",
        expr
    );
}
...
fn test_all_empty_list() {
    ...
    assert!(
        matches!(expr, Expression::Object(ref s) if s == "true" || s == "True"),
        "all(_, []) should be true: {:?}",
        expr
    );
}
...
fn test_all_true_case() {
    ...
    assert!(
        matches!(expr, Expression::Object(ref s) if s == "true" || s == "True"),
        "all(λ x . x > 0, [1,2,3]) should be true: {:?}",
        expr
    );
}
...
fn test_all_false_case() {
    ...
    assert!(
        result.is_ok(),
        "all(λ x . x > 0, [1,0,3]) should work: {:?}",
        result
    );

    let expr = result.unwrap();
    assert!(
        matches!(expr, Expression::Object(ref s) if s == "false" || s == "False"),
        "all(λ x . x > 0, [1,0,3]) should be false: {:?}",
        expr
    );
}
...
fn test_any_true_case() {
    ...
    assert!(
        matches!(expr, Expression::Object(ref s) if s == "true" || s == "True"),
        "any(λ x . x > 0, [0,0,3]) should be true: {:?}",
        expr
    );
}
...
fn test_any_false_case() {
    ...
    assert!(
        matches!(expr, Expression::Object(ref s) if s == "false" || s == "False"),
        "any(λ x . x > 0, [0,0,0]) should be false: {:?}",
        expr
    );
}
...
fn test_foldr_empty_list() {
    ...
    assert!(
        result.is_ok(),
        "foldr on empty list should work: {:?}",
        result
    );
...
fn test_foldr_sum() {
    ...
    let expr = result.unwrap();
    assert!(
        matches!(expr, Expression::Const(ref s) if s == "6"),
        "foldr(+, 0, [1,2,3]) should be 6: {:?}",
        expr
    );
}
...
fn test_builtin_product_alias() {
    ...
    assert!(
        matches!(expr, Expression::Const(ref s) if s == "24"),
        "builtin_product([2,3,4]) should be 24: {:?}",
        expr
    );
}
