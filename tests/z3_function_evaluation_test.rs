...
fn test_z3_compute_chained_functions() {
    ...
    // 1. Define square(x) = x * x as axiom
    let x = Int::fresh_const("x");
    let square_decl = FuncDecl::new("square", &[&Sort::int()], &Sort::int());

    let x_squared = &x * &x;
    let square_def = square_decl.apply(&[&x]).eq(&x_squared);
    solver.assert(&square_def);
    println!("   ✅ Defined: square(x) = x * x");

    // 2. Define sum_of_squares(a, b) = square(a) + square(b)
    let a = Int::fresh_const("a");
    let b = Int::fresh_const("b");
    let sum_decl = FuncDecl::new(
        "sum_of_squares",
        &[&Sort::int(), &Sort::int()],
        &Sort::int(),
    );

    let square_a = square_decl.apply(&[&a]);
    let square_b = square_decl.apply(&[&b]);

    // Convert Dynamic to Int for arithmetic
    let square_a_int = square_a.as_int().unwrap();
    let square_b_int = square_b.as_int().unwrap();
    let sum_body = &square_a_int + &square_b_int;

    let sum_def = sum_decl.apply(&[&a, &b]).eq(&sum_body);
    solver.assert(&sum_def);
    println!("   ✅ Defined: sum_of_squares(a, b) = square(a) + square(b)");

    // 3. Set concrete values: a=3, b=4
    let three = Int::from_i64(3);
    let four = Int::from_i64(4);

    // 4. Compute sum_of_squares(3, 4)
    let result_expr = sum_decl.apply(&[&three, &four]);

    println!("   🔍 Asking Z3 to compute sum_of_squares(3, 4)...");
    ...
    match solver.check() {
        SatResult::Sat => {
            ...
            let result_val = result_int.as_i64().unwrap();

            println!("   📊 Z3 computed: sum_of_squares(3, 4) = {}", result_val);
            println!("   🧮 Breakdown: 3² + 4² = 9 + 16 = 25");

            assert_eq!(result_val, 25, "Expected 3² + 4² = 25");
            println!("   ✅ VERIFIED: sum_of_squares(3, 4) = 25");
            println!("   🎉 Chained functions work - results used in other operations!");
        }
...
fn test_z3_compute_derived_operation() {
    ...
    let plus_decl = FuncDecl::new("plus", &[&Sort::int(), &Sort::int()], &Sort::int());
    let negate_decl = FuncDecl::new("negate", &[&Sort::int()], &Sort::int());
    let minus_decl = FuncDecl::new("minus", &[&Sort::int(), &Sort::int()], &Sort::int());

    // 1. Define plus and negate using built-in arithmetic
    let x = Int::fresh_const("x");
    let y = Int::fresh_const("y");

    // plus(x, y) = x + y (use Z3's arithmetic)
    let plus_def = plus_decl.apply(&[&x, &y]).eq(&(&x + &y));
    solver.assert(&plus_def);

    // negate(x) = -x (use Z3's negation)
    let neg_x = -&x;
    let negate_def = negate_decl.apply(&[&x]).eq(&neg_x);
    solver.assert(&negate_def);

    println!("   ✅ Defined primitive operations (plus, negate)");

    // 2. Define minus as derived operation: minus(x, y) = plus(x, negate(y))
    let neg_y = negate_decl.apply(&[&y]);
    let neg_y_int = neg_y.as_int().unwrap();
    let plus_result = plus_decl.apply(&[&x, &neg_y_int]);

    let minus_def = minus_decl.apply(&[&x, &y]).eq(&plus_result);
    solver.assert(&minus_def);
    ...
    let three = Int::from_i64(3);
    let result_expr = minus_decl.apply(&[&seven, &three]);
    ...
    match solver.check() {
        SatResult::Sat => {
            ...
            println!("   ✅ VERIFIED: Derived operation computed correctly!");
...
fn test_z3_grammar_v06_pattern() {
    ...
    // Setup Ring operations
    let plus = FuncDecl::new("plus", &[&Sort::int(), &Sort::int()], &Sort::int());
    let negate = FuncDecl::new("negate", &[&Sort::int()], &Sort::int());
    let minus = FuncDecl::new("minus", &[&Sort::int(), &Sort::int()], &Sort::int());

    // Map to Z3 built-ins
    let x = Int::fresh_const("x");
    let y = Int::fresh_const("y");
    solver.assert(plus.apply(&[&x, &y]).eq(&(&x + &y)));
    solver.assert(negate.apply(&[&x]).eq(&(-&x)));

    // DERIVED OPERATION (Grammar v0.6!)
    let neg_y = negate.apply(&[&y]);
    let neg_y_int = neg_y.as_int().unwrap();
    let minus_def = minus.apply(&[&x, &y]).eq(plus.apply(&[&x, &neg_y_int]));
    solver.assert(&minus_def);
    ...
    let four = Int::from_i64(4);
    let result1 = minus.apply(&[&ten, &four]);

    if solver.check() == SatResult::Sat {
        let model = solver.get_model().unwrap();
        let val_dyn = model.eval(&result1, true).unwrap();
        let val_int = val_dyn.as_int().unwrap();
        let val = val_int.as_i64().unwrap();
    ...
    let a = Int::fresh_const("a");
    let b = Int::fresh_const("b");
    let a_minus_b = minus.apply(&[&a, &b]);
    let a_minus_b_int = a_minus_b.as_int().unwrap();
    let result_plus_b = plus.apply(&[&a_minus_b_int, &b]);

    // Try to find counterexample where (a - b) + b ≠ a
    solver.assert(result_plus_b.eq(&a).not());
