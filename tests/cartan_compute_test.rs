...
fn test_solve_connection_minkowski() {
    ...
    println!("📊 Sample: {:?}", result.as_ref().map(|s| &s[..200.min(s.len())]));
    
    assert!(result.is_ok());
    assert!(conn_size < 50000, "Expression exploded! {} chars", conn_size);
}
...
fn test_solve_connection_schwarzschild() {
    ...
    assert!(result.is_ok());
    assert!(conn_size < 500000, "Expression exploded! {} chars", conn_size);
}
...
fn test_minkowski_curvature() {
    ...
    assert!(result.is_ok());
    assert!(curv_size < 100000, "Expression exploded! {} chars", curv_size);
}
...
fn test_schwarzschild_curvature() {
    ...
    // Step 1: Connection
    let conn = eval(&evaluator, "solve_connection(schwarzschild_tetrad(var(\"M\")))").unwrap();
    ...
    assert!(curv_size < 5000000, "Expression exploded! {} chars", curv_size);
}
...
fn test_minkowski_curvature_is_zero() {
    ...
    println!("📊 Minkowski curvature zero count: {} / {} components", zero_count, total_components);
    
    // Most components should be zero (allowing for some structural overhead)
    assert!(zero_count > 200, "Minkowski curvature should be mostly zeros, got {} zeros", zero_count);
}
...
fn test_schwarzschild_connection_nonzero() {
    ...
    let result = eval(&evaluator, "solve_connection(schwarzschild_tetrad(var(\"M\")))").unwrap();
    
    // Should contain the mass parameter M
    assert!(result.contains("EVariable(\"M\")"), "Schwarzschild connection should depend on mass M");
    
    // Should contain radial coordinate r
    assert!(result.contains("EVariable(\"r\")"), "Schwarzschild connection should depend on radius r");
    ...
    assert!(has_angular, "Schwarzschild connection should have angular dependence");
...
fn test_schwarzschild_curvature_structure() {
    ...
    // Should contain the mass parameter M
    assert!(result.contains("EVariable(\"M\")"), "Schwarzschild curvature should depend on mass M");
    
    // Should contain radial coordinate r (tidal forces ~ 1/r³)
    assert!(result.contains("EVariable(\"r\")"), "Schwarzschild curvature should depend on radius r");
    ...
    assert!(has_metric_factor, "Schwarzschild curvature should contain metric factor");
...
fn test_schwarzschild_curvature_component_r0101() {
    ...
    assert!(component.contains("EVariable(\"M\")") || component.contains("M"), 
            "R^0_1_01 should contain mass M");
    assert!(component.contains("EVariable(\"r\")") || component.contains("r"), 
...
fn test_schwarzschild_numerical_verification() {
    ...
    assert!(component.contains("EVariable(\"r\")"), "Should still contain r");
    assert!(!component.contains("EVariable(\"M\")"), "M should be substituted");
