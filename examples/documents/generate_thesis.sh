#!/bin/bash
# Generate PDF from jane_smith_thesis_kleisdoc.kleis
#
# For now, we generate a static Typst file since the evaluator
# needs more work to handle EditorNode data properly.
#
# Usage:
#   ./generate_thesis.sh
#   # Opens generated_kleisdoc_thesis.pdf

cd "$(dirname "$0")"

# Generate the Typst file
cat > generated_kleisdoc_thesis.typ << 'TYPST'
...
echo "Generated: generated_kleisdoc_thesis.typ"

# Compile to PDF
if command -v typst &> /dev/null; then
    typst compile generated_kleisdoc_thesis.typ
    echo "Generated: generated_kleisdoc_thesis.pdf"
    
    # Open PDF on macOS
    if [[ "$OSTYPE" == "darwin"* ]]; then
        open generated_kleisdoc_thesis.pdf
    fi
else
    echo "Typst not found. Install with: brew install typst"
    echo "Then run: typst compile generated_kleisdoc_thesis.typ"
fi
