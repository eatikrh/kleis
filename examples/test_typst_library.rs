...
fn main() {
    ...
    match compile_math_to_svg_with_ids(markup1, &placeholder_ids1) {
        Ok(output) => {
            ...
            println!("  Bounding boxes extracted: {}", output.argument_bounding_boxes.len());
            for (i, bbox) in output.argument_bounding_boxes.iter().enumerate().take(5) {
                println!("    {}. pos ({:.1}, {:.1}), size {:.1}x{:.1}, type: {}", 
                         i+1, bbox.x, bbox.y, bbox.width, bbox.height, bbox.content_type);
            }
        }
        Err(e) => {
