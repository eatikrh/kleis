...
We have shown that, given the background structure of a smooth oriented Lorentzian 4-manifold with its exterior algebra and Hodge star, the complete structure of classical electrodynamics follows from two axioms and the nilpotency $d^2 = 0$. The derivation chain is short:
...
The irreducible physical content is concentrated in a single equation: $d star.op F = star.op J$. This is the *only* axiom that connects the geometry (the field $F$) to the physical world (the source $J$). Everything else is either mathematical infrastructure (manifold, metric, exterior algebra) or a derivable consequence thereof.
...
We conclude with a philosophical observation. In the Projected Ontology framework, the distinction between 'law' and 'theorem' depends on whether a statement carries irreducible physical content or follows from mathematical structure. For electrodynamics, the inventory is clear: one physical equation ($d star.op F = star.op J$), one definitional axiom ($F = d A$), and a background manifold. Everything else --- gauge invariance, the homogeneous Maxwell equation, charge conservation, the absence of monopoles on trivial bundles --- is theorem. The universe does not 'obey' four independent Maxwell equations; it satisfies one physical constraint on a Lorentzian manifold, and the rest is the compiled output of the exterior algebra.
