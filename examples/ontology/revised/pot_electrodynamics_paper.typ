...
#align(center)[
  #rect(width: 85%, stroke: none)[
    #align(left)[
      ...
      #v(0.3em)
      #text(size: 10pt)[We show that the complete structure of classical electrodynamics --- gauge invariance, the absence of magnetic monopoles, and charge conservation --- follows as a chain of one- and two-step derivations from two axioms: the definition $F = d A$ (field strength is the exterior derivative of the potential) and the inhomogeneous Maxwell equation $d star.op F = star.op J$ (relating the field to its sources via the Lorentzian Hodge star). The sole algebraic ingredient is the nilpotency of the exterior derivative, $d^2 = 0$, which is not a physical assumption but a structural property of the exterior algebra on any smooth manifold. We formalize these derivations on the Kleis verification platform, where every axiom is explicit, every theorem is machine-checked by the Z3 SMT solver, and the entire exterior calculus --- wedge product, Hodge star, interior product, Lie derivative, and de Rham cohomology --- is axiomatized in the standard library. The electromagnetic structures share the same Cartan geometry substrate used in our companion papers on flat galactic rotation curves and quantum entanglement, where the admissible kernel framework produces gravitational and quantum phenomena from the same projection operator. The present paper adds the gauge-theoretic sector: we derive that the potential $A$ is a projection degree of freedom with no ontological weight, that monopoles are topologically forbidden by the manifold's de Rham cohomology, and that charge conservation is the Noether invariant of the $U(1)$ gauge symmetry. We further show that the Einstein--Maxwell coupling emerges from the same anholonomy coefficients that govern spacetime curvature, and that the non-abelian Yang--Mills extension $F = d A + A and A$ is already formalized in the same library, setting the stage for a verified treatment of the strong and electroweak interactions. The result is a demonstration that the 'laws' of electrodynamics are not empirical regularities but theorems of the projected ontology --- the compiled output of structural constraints on a Lorentzian manifold.]
    ]
  ]
]
...
We propose a stronger claim: Maxwell's equations are *theorems*. Given a 4-dimensional Lorentzian manifold, a potential 1-form $A$, and the definition $F = d A$ of the field strength as an exact 2-form, the entire structure of classical electrodynamics follows from a single algebraic fact --- the nilpotency of the exterior derivative, $d^2 = 0$ --- plus one irreducible physical axiom: the inhomogeneous equation $d star.op F = star.op J$ relating the field to its sources.

From these two statements, we derive:
...
Magnetic monopoles, if they existed, would correspond to $F$ being closed but *not* exact --- a nontrivial element of $H^2_("dR")(M)$. On $RR^4$ (or any contractible manifold), $H^2_("dR") = 0$, and monopoles are topologically impossible. Their non-existence is a theorem of the manifold's topology, not a postulate of physics.

In a manifold with nontrivial $H^2$ (e.g., $RR^3 backslash {0}$, which has a point removed), monopoles become topologically possible. The Dirac monopole lives precisely in this cohomological gap. But on the standard Minkowski spacetime of classical electrodynamics, the topology forbids them.
...
This result is not a separate law. It is a two-step consequence of the inhomogeneous Maxwell equation and $d^2 = 0$. No additional axiom is needed.

The Noether perspective deepens the result. Noether's theorem states that every continuous symmetry of the action yields a conserved current. The gauge transformation $A arrow.r A + d chi$ is a $U(1)$ symmetry (Section 4), and the associated conserved current is precisely $J^mu$. Charge conservation is therefore the *Noether invariant* of the gauge symmetry.

The logical chain is:

$ d^2 = 0 arrow.r.double "gauge invariance" arrow.r.double U(1) "symmetry" arrow.r.double "conserved current" J^mu arrow.r.double partial_mu J^mu = 0 $

Charge is not an arbitrary 'stuff' assigned to particles. It is a geometric quantity forced into existence by the structure of the exterior algebra. If you have a potential 1-form $A$ on a smooth manifold, and you define $F = d A$, then gauge symmetry is automatic, and a conserved current must exist. The source term $J$ in $d star.op F = star.op J$ is not a free choice --- it is constrained by $d(star.op J) = 0$.

This is the closing argument of the paper: two axioms and the nilpotency of $d$ yield the complete classical electrodynamics. The field equations, the gauge symmetry, the topological structure, and the conservation laws are all derivable. Nothing is added by hand.
...
The coupling is deeper than the equation suggests. In the Cartan formalism, both gravity and electromagnetism arise from the *same* mathematical structures:
...
The connection $omega^a_b$ and the potential $A$ are both 1-forms. The curvature $R^a_b$ and the field strength $F$ are both 2-forms. The Bianchi identity $d R + omega and R - R and omega = 0$ and the homogeneous Maxwell equation $d F = 0$ are both consequences of $d^2 = 0$. The anholonomy coefficients that determine the gravitational connection also constrain the electromagnetic field through the coupled Einstein--Maxwell equation.

This is the structural bridge between our companion papers: the same Cartan geometry substrate that produces flat galactic rotation curves (from the gravitational sector of the admissible kernel) and Bell inequality violation (from the measurement sector) also determines classical electrodynamics. The admissibility pipeline is the same; only the sector changes.
...
We have shown that classical electrodynamics --- in its entirety --- follows from two axioms and the nilpotency of the exterior derivative. The derivation chain is short:
...
The irreducible physical content is concentrated in a single equation: $d star.op F = star.op J$. This is the *only* axiom that connects the geometry (the field $F$) to the physical world (the source $J$). Everything else is mathematics.
...
We conclude with a philosophical observation. In the Projected Ontology framework, the 'laws of nature' are not empirical regularities observed in data and codified in equations. They are *theorems* --- logical necessities of the mathematical structures that the ontology selects. The universe does not 'obey' Maxwell's equations; rather, any consistent projection of a $U(1)$ gauge field onto a Lorentzian manifold *must* satisfy them. The equations are the compiled output of the structure.
