...
#align(center)[
  #rect(width: 85%, stroke: none)[
    #align(left)[
      ...
      #v(0.3em)
      #text(size: 10pt)[We present machine-verified derivations of Bell inequality violation and GHZ contextuality within Projected Ontology Theory (POT), without invoking superluminal influences or noncontextual hidden-variable assignments. In POT, entangled particles are not separate systems connected by a mysterious influence — they are spatial manifestations of a single, non-separable ontological wave viewed through different projection angles. The measurement kernel is a sector of the same admissible Green's kernel that governs gravitational physics (shown in our companion paper on flat rotation curves). Our central result is that the singlet correlation E(a,b) = −cos(θ) is not postulated but *derived* from five structural assumptions about the ontological Hilbert space $cal(H)_("ont")$: (A) SU(2) symmetry acts on ontic flows, (B) the projection operator is an SU(2) intertwiner (symmetry survives projection), (C) inner products are SU(2)-invariant, (D) the singlet state is invariant under diagonal SU(2), and (E) probability is governed by a Born-type modal visibility rule. The empirical content is thus encoded in ontic SU(2) symmetry together with its survival under projection, rather than in a correlation formula. From these axioms we prove using the Z3 SMT solver: (1) the singlet-state correlation follows as a representation-theoretic lemma, (2) Bell's inequality holds for all separable states, (3) Bell's inequality is violated for non-separable states, (4) the violation reflects rejection of separability rather than locality, and (5) operational no-signaling holds despite ontic contextuality — marginal statistics at each detector are independent of the remote setting. The CHSH parameter reaches S = 2√2, matching quantum mechanics exactly. We further extend the analysis to three-particle GHZ states, proving that no noncontextual assignment satisfies the GHZ parity constraints (Z3 UNSAT), while POT's context-dependent outcomes satisfy all four constraints simultaneously. Every step is formally verified and reproducible from the accompanying source files.]
    ]
  ]
]
...
We now present the central theoretical advance of this paper: the derivation of the singlet correlation $E(a, b) = -cos(theta)$ from structural symmetry principles, rather than its postulation as an axiom. The derivation rests on five interlocking axioms (A–E below) together with kernel admissibility (Section 2), each of which has clear physical content and can be independently motivated. *Assumption ledger:* the empirical content is not encoded as a correlation formula; it is encoded as the claim that $cal(H)_("ont")$ carries SU(2) symmetry and that this symmetry survives projection. The cosine law is downstream of representation theory, not an input.
...
For a spin-½ system, the $K_("rep")$ sector acts on the $CC^2$ spinor representation. The singlet state in $cal(H)_("ont")$ corresponds to:
...
What is *proved* in this paper and its companion: the kernel composition axiom ensures that the combined operator remains admissible, so all foundational guarantees (linearity, nullspace closure, projective equivalence) carry through to both sectors simultaneously. Both sectors share admissibility under composition — this is a theorem.

What remains *conjectural*: whether a single non-trivial structural constraint forces both the logarithmic Green's function of the gravitational sector and the SU(2) inner-product structure of the measurement sector. The factorization $K = K_("univ") dot K_("dyn") dot K_("rep")$ is compatible with both, but the deeper question — whether admissibility *uniquely selects* these two sectors — is open. We regard this as a productive research direction, not a claim of this paper.
...
- The correlation $E(a, b) = -cos(2 theta)$ for polarization. The factor of 2 arises because linear polarization angles on the Poincaré sphere cover $S^2$ with a $2:1$ map ($theta_("pol") arrow.r 2 theta_("pol")$ on the Bloch sphere). Our spin-½ formalism produces $-cos(theta)$ directly; the structures are identical up to this geometric rescaling.
...
== Skolemization of Quantified Axioms

Two axioms in our formalization — the basis spanning axiom (V2) and the irreducibility axiom (V6) — require existentially quantified witnesses inside universal quantifiers:

$ forall(s). exists(alpha, beta). s = alpha e_(arrow.t) + beta e_(arrow.b) $
$ forall(s != 0). exists(g in "SU"(2)). forall(c in CC). R(g, s) != c dot s $

Z3's quantifier reasoning uses *E-matching*: it instantiates universally quantified axioms only when it encounters a ground term matching a trigger pattern (typically a function application involving the quantified variable). In both axioms above, the universally quantified variable $s$ appears bare — not inside a function call — so Z3 cannot find a trigger to instantiate the axiom. The result is that Z3 returns `Unknown` after exhausting its timeout, even though the axioms are present in the solver context.

We resolve this by *Skolemization*: replacing each existentially quantified variable with an explicit function of the enclosing universal variables. For V2, we introduce coordinate projections $"coord_up"(s)$ and $"coord_down"(s)$:

$ forall(s). s = "spinor_add"("spinor_smul"("coord_up"(s), e_(arrow.t)), thin "spinor_smul"("coord_down"(s), e_(arrow.b))) $

For V6, we introduce an irreducibility witness $"irred_witness"(s)$:

$ forall(s != 0). forall(c in CC). R("irred_witness"(s), s) != "spinor_smul"(c, s) $

This transformation is logically equivalent: by the Skolem normal form theorem, replacing $forall x. exists y. P(x,y)$ with $forall x. P(x, f(x))$ for a fresh function symbol $f$ preserves satisfiability and validity. No mathematical content is added or removed — the Skolem functions are implicit in the original statement (they are exactly the choice functions guaranteed by the axiom of choice). The only change is that the witnesses are *named*, giving Z3 the function-application terms it needs for E-matching.

After Skolemization, both axioms verify instantly (< 200ms). This is a general technique applicable to any $forall exists$ axiom in SMT verification.
...
Structurally, the measurement kernel and the gravitational kernel are both sectors of the factorized operator $K = K_("univ") dot K_("dyn") dot K_("rep")$, sharing the same admissibility axioms. This paper and its companion prove that admissibility under composition carries foundational guarantees to both sectors. Whether a deeper structural constraint *forces* both the gravitational and measurement sectors remains an open and productive question — not a claim of the present work.

We emphasize what this paper is and is not. It is a formally verified, internally consistent ontological reinterpretation of quantum entanglement, grounded in SU(2) symmetry survival through projection. It reproduces quantum mechanical predictions exactly — it does not modify them. The question of whether POT predicts deviations from standard QM at some scale, or whether it derives the SU(2) structure itself from deeper principles, remains open. The present contribution is the demonstration that the full entanglement phenomenology (singlet correlation, Bell violation, Tsirelson's bound, GHZ parity constraints, and no-signaling) can be machine-verified from explicit structural axioms — with the assumption moved from correlation formula to ontic symmetry.

Extending to $N$-particle systems, quantum teleportation protocols, and other gauge symmetries (SU(3), Lorentz) remain open directions.
...
*V2.* `basis_decomposition`: $forall(s). s = "coord_up"(s) e_(arrow.t) + "coord_down"(s) e_(arrow.b)$ \
...
\
*Irreducibility:* \
*V6.* `R_irreducible`: $forall(s != 0). R("irred_witness"(s), s) != c dot s$ for any scalar $c$ \
\
