...
#align(center)[
  #rect(width: 85%, stroke: none)[
    #align(left)[
      ...
      #v(0.3em)
      #text(size: 10pt)[We present machine-verified derivations of Bell inequality violation and GHZ contextuality from first principles within Projected Ontology Theory (POT), requiring neither non-locality nor hidden variables. In POT, entangled particles are not separate systems connected by a mysterious influence — they are spatial manifestations of a single, non-separable ontological wave viewed through different projection angles. The measurement kernel is a sector of the same admissible Green's kernel that governs gravitational physics (shown in our companion paper on flat rotation curves). Our central result is that the singlet correlation $E(a,b) = -cos(theta)$ follows from five structural axioms together with the representation theory of SU(2); its consequences are machine-verified. The axioms are: (A) SU(2) symmetry lives in the ontological Hilbert space $cal(H)_("ont")$, (B) the projection operator is an SU(2) intertwiner (symmetry survives projection), (C) inner products are SU(2)-invariant, (D) the singlet state is invariant under diagonal SU(2), and (E) probability is governed by a Born-type modal visibility rule. The structural axioms constrain the correlation to a function of relative angle; the irreducibility of the 2D spinor representation pins that function to cosine (a representation-theoretic fact); and Z3 machine-verifies all downstream consequences. The result: (1) the cosine that appears in $E(a,b) = -cos(theta)$ is not imported from quantum mechanics — it is the invariant scalar of projection geometry with rotational symmetry, a geometric fact known since antiquity, (2) Bell's inequality holds for all separable (product) states, (3) Bell's inequality is violated for non-separable states, (4) this violation is legitimate because POT rejects separability, not locality, and (5) operational no-signaling holds despite ontic contextuality — marginal statistics at each detector are independent of the remote setting. The CHSH parameter reaches $S = 2 sqrt(2)$, matching quantum mechanics exactly. We further extend the analysis to three-particle Greenberger-Horne-Zeilinger (GHZ) states, proving (6) that no noncontextual hidden-variable assignment satisfies the GHZ parity constraints (Z3 UNSAT), while (7) POT's context-dependent projection outcomes satisfy all four parities simultaneously. This demonstrates that POT is not a noncontextual hidden-variable theory — it belongs to a strictly larger class than those refuted by the GHZ no-go theorem. Every step is formally verified and reproducible from the accompanying source files.]
    ]
  ]
]
...
A key contribution of this paper is the *derivation* of the singlet correlation $E(a,b) = -cos(theta)$ from deeper structural principles, rather than its assertion as an axiom. We show that SU(2) symmetry lives in the ontological Hilbert space $cal(H)_("ont")$, that this symmetry *survives* the projection to observable spacetime (the projection operator is an SU(2) intertwiner), and that the cosine correlation follows as a representation-theoretic consequence of singlet invariance and the Born-type modal visibility rule. Crucially, the cosine that appears is not a quantum-mechanical artifact — it is the invariant scalar of projection geometry with rotational symmetry, a geometric fact that predates quantum theory by millennia. This moves the explanatory burden from 'the correlation happens to be $-cos(theta)$' to 'the ontic space carries rotational projection structure and the projection respects it' — a far more physically grounded starting point.
...
== Derivation: From SU(2) to $-cos(theta)$
...
No step in this derivation assumes the correlation formula. It is important to be precise about the division of labor. Steps 1–5 are structural: the axioms constrain $E(a,b)$ to be a function of $theta$ alone, determined on a single orbit. These steps are machine-verified by Z3. Step 6 is representation-theoretic: it invokes the classification of invariant bilinear forms on irreducible SU(2) representations (Schur's lemma and the Wigner-Eckart theorem) to identify that function as $cos(theta)$. This is standard mathematical physics, but it is not a first-order derivation — it is a theorem about compact group representations that the solver accepts as a structural input. Z3 then verifies all downstream consequences: Bell violation, CHSH saturation, no-signaling, and GHZ contextuality. The derivation is thus a hybrid of structural constraint (machine-verified) and representation classification (mathematical fact), grounded in POT's ontology rather than the standard Hilbert-space formalism.
...
== The Ontological Status of Cosine

A natural question arises: where does the cosine in $E(a,b) = -cos(theta)$ come from? The derivation in Section 4.6 traces it to representation theory — the unique SU(2)-invariant bilinear form on a 2D irreducible representation. But this mathematical genealogy may obscure a deeper point. Cosine does not originate in quantum mechanics. It does not require Hilbert space, operator algebra, or Wigner $D$-matrices. Its definition is elementary: in a right triangle with unit hypotenuse, $cos(theta)$ is the length of the projection onto the adjacent side. It is the scalar that answers: _how much of this direction lies along that direction?_ Pythagoras had the essential content two millennia before Dirac.

This observation has philosophical consequences for POT. In the standard narrative, cosine appears in the singlet correlation because quantum mechanics employs Hilbert space, which employs inner products, which yield cosines. The correlation looks like an artifact of the formalism. In POT, the causal order is reversed: projection geometry is ontologically prior, and quantum mechanics inherits its angular structure from it. The ontological Hilbert space $cal(H)_("ont")$ carries an invariant inner product and a rotation symmetry — not because it is a Hilbert space in the quantum-mechanical sense, but because the projection structure requires a notion of _directional overlap_. Once there is an invariant inner product and a compact rotation group, the invariant scalar relating two directions must be cosine. This is a theorem about rotation geometry, not about quantum theory.

To be clear: the representation-theoretic derivation in Section 4.6 and the geometric projection interpretation here are not competing explanations. They are two descriptions of the same invariant structure — the unique scalar preserved by compact rotational symmetry in a 2D unitary space. SU(2) _is_ the symmetry group of that projection geometry; representation theory classifies its invariants; projection geometry motivates them physically. They converge on the same object.

The philosophical shift can be stated precisely. We are not claiming that cosine as an analytic function (defined by power series, differential equation, or the exponential of an imaginary unit) is an ontological primitive. We are claiming that _projection geometry with rotational symmetry_ is ontological. Cosine is simply the scalar encoding of that geometry — the ratio that any metric triangle must satisfy. If one accepts that distances, angles, and projection relations are features of the ontological structure, then cosine is as inevitable as arithmetic. It is not imported from physics; it is imported from geometry.

This reframing clarifies several features of our formalization. First, the ground cosine instances used in verification — $cos(0) = 1$, $cos(pi\/2) = 0$, $cos(pi) = -1$ — are not importing quantum-mechanical content. They are metric facts about specific angular separations, geometric truths that hold in any space with rotational invariance. Second, the Z3 solver's inability to derive cosine from abstract SU(2) axioms is not a limitation of the solver but a reflection of a genuine epistemic boundary: representation _classification_ is analytic, while the geometric invariant is structural. By asserting the projection scalar directly, we are not smuggling in an assumption — we are encoding the geometric content that the symmetry axioms alone cannot manufacture. Third, the minimal ontological commitment is weaker than full Euclidean geometry. What is required is: a rotationally invariant projection scalar, periodicity, normalization at zero angle, and boundedness. Cosine is the canonical model of these constraints, but the framework does not preclude non-Euclidean generalizations — a point that may become relevant when the gravitational sector introduces curvature.

The cosine does not arise because we assume Hilbert space; it arises because we assume projection geometry with compact rotational symmetry. Hilbert space is one model of that structure, not its source.

The deeper consequence is that entanglement correlation becomes a statement of geometry rather than of quantum weirdness: $E(a,b) = -cos(theta)$ says that the correlation between two projections of the same invariant mode is minus the projection scalar of their relative orientation. The minus sign comes from singlet antisymmetry; the cosine comes from the fact that projection geometry has rotational invariance. The mystery of entanglement is thereby relocated from the scalar (which is ancient geometry) to the non-separability (which is the genuinely novel ontological claim). This separation of concerns — familiar geometry carrying a novel ontological structure — is one of the conceptual advantages of the POT framework.
...
We emphasize that the entanglement derivation in this paper does not *depend* on the gravitational sector — the SU(2) symmetry axioms and kernel admissibility are sufficient on their own. The kernel unification is architectural: it provides the broader structural motivation for why the same admissibility axioms appear in both sectors, and it constrains future extensions. The kernel composition axiom ensures that the combined operator remains admissible, so all foundational guarantees (linearity, nullspace closure, projective equivalence) carry through to both sectors simultaneously. Whether this architectural compatibility reflects a deeper structural necessity is an open question for future work.
...
We address several objections that a critical reader may raise.

*"SU(2) already encodes quantum structure."* SU(2) is the double cover of the rotation group SO(3). It describes the symmetry of angular momentum in classical mechanics, crystallography, and rigid-body dynamics — none of which are quantum theories. Positing SU(2) symmetry in $cal(H)_("ont")$ is an ontological commitment to rotational structure in the pre-observable space, not a covert importation of quantum mechanics. What is quantum-specific is the *projection* from $cal(H)_("ont")$ to observable spacetime and the Born-type visibility rule (Axiom E). The symmetry is geometric; the measurement theory is projective.

*"The irreducibility assumption encodes the answer."* Irreducibility of the spinor representation $R$ is a structural constraint, not an empirical fitting parameter. It says: the observable spin space has no invariant substructure — there is no 'hidden direction' that all rotations preserve. This is the minimal non-trivial representation of SU(2) and corresponds to the physical observation that spin-½ particles have exactly two distinguishable states. Without irreducibility, the correlation function would be underdetermined (any Legendre polynomial $P_j$ would be consistent). With it, the function is uniquely $cos(theta)$. The assumption is thus a structural selection principle, analogous to requiring that the integers satisfy the Peano axioms rather than some weaker fragment.

*"Why SU(2) rather than another compact group? Why 2D rather than higher?"* SU(2) is selected because the ontological claim is that $cal(H)_("ont")$ carries angular momentum symmetry — the same rotational structure observed in spin, polarization, and orbital mechanics. Among compact Lie groups, SU(2) is the unique simply connected double cover of the 3D rotation group SO(3); it is not an arbitrary choice but the minimal group that captures half-integer spin. The 2D irreducible representation is selected because this paper treats spin-½ systems (photon polarization, electron spin): systems with exactly two distinguishable states. For spin-$j$ systems, the representation would be $(2j+1)$-dimensional, and the correlation function would involve the Legendre polynomial $P_j(cos theta)$ rather than bare $cos(theta)$. The present framework generalizes naturally to higher spin; we restrict to $j = 1\/2$ because it is the experimentally relevant case for Bell and CHSH tests.

*"Standard QM is a model of your axioms — so what is new?"* That standard quantum mechanics satisfies our axioms is a feature, not a bug: it is the existence proof that the axiom set is satisfiable. What is new is the *ontological interpretation* and the *derivational direction*. In standard QM, the singlet state is defined in a tensor product Hilbert space, and the $-cos(theta)$ correlation is computed from the trace formula. In POT, the correlation is derived from symmetry survival through projection, non-separability is ontological rather than formal, and the measurement kernel connects to the gravitational sector through shared admissibility. The axioms are the same mathematical constraints; the ontology — what the symbols refer to — is different. Whether this ontological reinterpretation yields new predictions is an open question that depends on extending POT to regimes where it diverges from standard QM (e.g., the interface between gravitational and quantum sectors).
...
This matters because it addresses the most natural criticism of the previous version: 'You just assumed the answer.' Now the answer follows from symmetry, and the symmetry has clear physical content — it is the claim that $cal(H)_("ont")$ carries the same angular momentum structure that manifests in observable physics. The assumption has moved from formula to structure. Moreover, the cosine that emerges is not a quantum artifact but the invariant scalar of projection geometry — the same geometric fact expressed by the ratio of adjacent side to hypotenuse in a right triangle. The mystery of entanglement is thereby relocated from the correlation function (which is ancient geometry) to the non-separability of the ontological mode (which is the genuinely novel claim).
