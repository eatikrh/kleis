...
#figure(
  image("moonlight_score.pdf", width: 100%),
  caption: [Beethoven's Moonlight Sonata (Op. 27 No. 2, measures 1--14). Three-layer texture: melody with stems up entering at measure 5, continuous triplet arpeggiation with stems down, and sustained bass. Generated by Kleis + LilyPond from the same typed AST that the axiom checkers verify.]
) <fig:moonlight-score>
...
- $"pitch_to_midi"(p)$: absolute semitone number (C0 = 0, middle C = 48).

- $"pitch_class"(p) = "mod_int"("pitch_to_midi"(p), 12)$: the pitch class modulo octave equivalence, where $"mod_int"(a, b) = a - "floor"(a slash b) times b$.

- $"interval_class"(p_1, p_2)$: the interval between two pitches reduced to the range 0--11 (so an octave is 0, a fifth is 7).
