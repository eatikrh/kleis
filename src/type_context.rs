...
use crate::kleis_ast::{
    Program, TopLevel, StructureDef, StructureMember, 
    ImplementsDef, ImplMember, TypeExpr
};
...
impl TypeContextBuilder {
    ...
    fn register_toplevel_operation(&mut self, _op_decl: &crate::kleis_ast::OperationDecl) -> Result<(), String> {
    ...
    fn type_expr_to_string(&self, type_expr: &TypeExpr) -> String {
        match type_expr {
            ...
            TypeExpr::Function(from, to) => {
                format!("{} → {}", self.type_expr_to_string(from), self.type_expr_to_string(to))
            }
    ...
    pub fn structures_for_type(&self, type_name: &str) -> Vec<String> {
        self.registry.type_to_structures
            .get(type_name)
            .cloned()
    ...
    pub fn suggest_operation(&self, type_name: &str, attempted_operation: &str) -> Option<String> {
        ...
        if available_ops.is_empty() {
            return Some(format!("{} implements {:?} but no operations available", type_name, structures));
        }
