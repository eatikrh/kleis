...
impl<'r> AxiomVerifier<'r> {
    ...
    fn ensure_structure_loaded(&mut self, structure_name: &str) -> Result<(), String> {
        ...
        // through FFI frames triggers the C++ abort).
        {
            let limit = crate::solvers::z3::backend::z3_memory_limit_bytes();
            if limit > 0 {
                let used = z3::get_estimated_alloc_size();
                if used > limit {
                    return Err(format!(
                        "Z3 memory limit reached ({}MB / {}MB) while loading structure '{}'. \
                         Set KLEIS_Z3_MEMORY_MB to increase the limit.",
                        used / (1024 * 1024),
                        limit / (1024 * 1024),
                        structure_name,
                    ));
    ...
    fn load_axioms_recursive(
        ...
        let mem_limit = crate::solvers::z3::backend::z3_memory_limit_bytes();

        for member in members {
            // Check memory before each Z3 operation to bail before OOM.
            // Use 90% threshold: Z3 can allocate large chunks in a single
            // operation, so checking at 100% is too late.
            if mem_limit > 0 {
                let used = z3::get_estimated_alloc_size();
                if used > mem_limit * 9 / 10 {
                    return Err(format!(
                        "Z3 memory limit approaching ({}MB / {}MB). \
                         Aborting axiom loading to prevent OOM crash. \
                         Set KLEIS_Z3_MEMORY_MB to increase.",
                        used / (1024 * 1024),
                        mem_limit / (1024 * 1024),
                    ));
                }
