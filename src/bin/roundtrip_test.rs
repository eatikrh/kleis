...
fn main() {
    ...
    let tests = vec![
        ...
        // ===== PHYSICS EQUATIONS =====
        ("Einstein field eqs", r"G_{{\mu\nu}} + \Lambda \, g_{{\mu\nu}} = \kappa \, T_{{\mu\nu}}"),
        ("Maxwell tensor", r"F_{{\mu\nu}} = \partial_{{\mu}} A_{{\nu}} - \partial_{{\nu}} A_{{\mu}}"),
        ...
        ("Second derivative", r"\frac{\partial^{2} \,V}{\partial x^{2}}"),
        ...
        ("Double integral", r"\iint_{D} f(x,y) \, \mathrm{d}x \, \mathrm{d}y"),
        ("Triple integral", r"\iiint_{V} f(x,y,z) \, \mathrm{d}x \, \mathrm{d}y \, \mathrm{d}z"),
        ("Sum notation", r"\sum_{ n=1 }^{ \infty } \frac{1}{n^{2}}"),
        ("Product notation", r"\prod_{ i=1 }^{ n } i"),
        
        // ===== COMPLEX EQUATIONS =====
        ("Riemann zeta", r"\zeta(s) = \sum_{ n=1 }^{ \infty } \frac{1}{n^{s}}"),
        ...
        ("Pmatrix 3x3", r"\begin{pmatrix}1&0&0\\0&1&0\\0&0&1\end{pmatrix}"),
        ("Vmatrix 2x2", r"\begin{vmatrix}a&b\\c&d\end{vmatrix}"),
        ("Vmatrix 3x3", r"\begin{vmatrix}1&2&3\\4&5&6\\7&8&9\end{vmatrix}"),
    ...
    if success_count == tests.len() {
        println!("\n🎉 Perfect! All {} test cases parse successfully!", tests.len());
        println!("   Parser coverage matches all render.rs test patterns!");
    } else {
        println!("\n⚠️  {} expressions failed to parse.", tests.len() - success_count);
