...
fn main() {
    ...
    let templates = vec![
        // Basic Operations
        ("Fraction", template_fraction as fn() -> kleis::ast::Expression),
    ...
    for (name, template_fn) in &templates {
        ...
        let category = if name.contains("Matrix") || name.contains("PMatrix") || name.contains("VMatrix") {
            "Matrices"
        } else if name.contains("Ket") || name.contains("Bra") || name.contains("Commutator") || name.contains("Expectation") {
            "Quantum"
        } else if name.contains("Vector") || name.contains("Dot Product") || name.contains("Cross") || name.contains("Norm") || name.contains("Absolute") {
            "Vectors"
        } else if name.contains("sin") || name.contains("cos") || name.contains("tan") || name.contains("Log") || name.contains("Exponential") {
            "Functions"
        } else if name.contains("Accent") || name.contains("Hat") || name.contains("Bar") || name.contains("Tilde") {
            "Accents"
        } else if name.contains("Christoffel") || name.contains("Riemann") || name.contains("Tensor") {
            "Tensors"
        } else if name.contains("Integral") || name.contains("Sum") || name.contains("Product") || name.contains("Derivative") || name.contains("Gradient") || name.contains("Limit") {
            "Calculus"
        } else {
            "Basic"
        };
        ...
        match compile_with_semantic_boxes(&expr, &placeholder_ids) {
            Ok(output) => {
                ...
                if has_placeholders || has_boxes {
                    println!("✅ PASS ({} ph, {} boxes)", output.placeholder_positions.len(), output.argument_bounding_boxes.len());
                    passed += 1;
                    ...
                    println!("⚠️  WARN (expected {} ph, got {})", placeholders.len(), output.placeholder_positions.len());
                    warnings += 1;
                }
            }
            Err(e) => {
    ...
    if failed == 0 {
        ...
        println!("\n⚠️  {} templates failed. Please review errors above.", failed);
        std::process::exit(1);
    }
}
