...
pub fn expression_to_typst(
    ...
    match expr {
        ...
        Expression::Operation { name, args } => {
            operation_to_typst(name, args, ctx)
        }
    }
}

/// Convert an operation to Typst math markup
fn operation_to_typst(
    name: &str,
    args: &[Expression],
    ctx: &mut ConversionContext,
) -> String {
...
mod tests {
    ...
    fn test_convert_fraction_with_placeholder() {
        let expr = Expression::Operation {
            ...
            args: vec![
                Expression::Placeholder { id: 1, hint: "num".to_string() },
        ...
        let expr = Expression::Operation {
            ...
            args: vec![
                Expression::Operation {
                    name: "plus".to_string(),
                    args: vec![
                        Expression::Placeholder { id: 1, hint: "a".to_string() },
                        Expression::Object("x".to_string()),
                    ],
                },
                Expression::Placeholder { id: 2, hint: "b".to_string() },
            ],
        };
        let mut ctx = ConversionContext::new();
        let markup = expression_to_typst(&expr, &mut ctx);
