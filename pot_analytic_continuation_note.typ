...
= The Primes and the Zeta Function


The cleanest example of the entire framework comes not from physics but from number theory.

The prime numbers are ontology.  They are just _there_ --- 2, 3, 5, 7, 11, 13 --- observable, countable, verifiable.  You can check that 7 is prime by trying to divide it.  No theory is needed.  They are the observational data, the restricted domain, the input.

Euler showed that this discrete list of numbers encodes a function:
$ zeta(s) = product_p 1/(1 - p^(-s)) $
The Euler product is built entirely from the primes.  It is a faithful encoding of the list.  But then you analytically continue $zeta(s)$ to the full complex plane, and suddenly the function has a pole at $s = 1$, trivial zeros at negative even integers, non-trivial zeros on (conjecturally) the critical line $"Re"(s) = 1/2$, a functional equation relating $zeta(s)$ to $zeta(1-s)$, and deep connections to random matrix theory.

_None of that structure is in the primes._  It was created by the continuation.
...
#table(
  ...
  align: (left, center, left),
  stroke: none,
  [Prime numbers], [$arrow.r.long$], [Input (ontology)],
  [Euler product + analytic continuation], [$arrow.r.long$], [Production kernel $K$],
  [Zeros, pole, functional equation], [$arrow.r.long$], [Artifacts of continuation],
  [Prime distribution, counting function], [$arrow.r.long$], [Observable projection $Q$],
)

The primes do not need the zeta function.  They were there before Euler, before Riemann.  But the zeta function needs the primes --- it is built from them.

What makes this example deeper than the physics cases is what happens next.  The explicit formula for the prime counting function $pi(x)$ expresses the distribution of primes in terms of the zeros of $zeta(s)$.  The structure that the continuation _created_ --- the zeros on the critical line --- turns out to _explain_ the data it was continued from.  The Riemann Hypothesis itself is a statement about the artifacts of continuation, not about the primes directly.  And yet it controls the primes completely.

The continuation does not just extend --- it explains.  The primes are the ontology.  The zeros are in the continuation.  And the explanation of why the primes are distributed the way they are lives in $ker(Q)$ --- in structure that is not itself a prime number, but that governs all prime numbers.

This is exactly the relationship between physics and physical theory.  Particles and measurements are ontology.  Lagrangians and field equations are the continuation.  And the explanation of why the observations are the way they are lives in the structure that the continuation created.
...
+ Reveals that the Riemann zeta function is the analytic continuation of the primes, and the Riemann Hypothesis is a statement about the artifacts of continuation
