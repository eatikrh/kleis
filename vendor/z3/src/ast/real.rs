...
impl Real {
    ...
    pub fn from_rational(num: i64, den: i64) -> Real {
        // Z3_mk_real takes c_int (i32). For values that fit, use the fast path.
        // For larger values, fall back to string-based construction to avoid
        // silent truncation (e.g., 14_135_000_000 wrapping to -454_934_592).
        let i32_min = i32::MIN as i64;
        let i32_max = i32::MAX as i64;
        if num >= i32_min && num <= i32_max && den >= i32_min && den <= i32_max {
            let ctx = &Context::thread_local();
            unsafe {
                Self::wrap(ctx, {
                    Z3_mk_real(
            ...
            Self::from_rational_str(&num.to_string(), &den.to_string())
                .expect("from_rational: string construction failed")
        }
    }
