...
impl Dynamic {
    ...
    pub fn new_const<S: Into<Symbol>>(name: S, sort: &Sort) -> Self {
        ...
        unsafe {
            let raw = Z3_mk_const(ctx.z3_ctx.0, name.into().as_z3_symbol(), sort.z3_sort);
            match raw {
                Some(ptr) => Self::wrap(ctx, ptr),
                None => {
                    eprintln!("   ⚠️  Z3 out of memory (const allocation failed). Exiting.");
    ...
    pub fn fresh_const(prefix: &str, sort: &Sort) -> Self {
        ...
        unsafe {
            ...
            let p = pp.as_ptr();
            let raw = Z3_mk_fresh_const(ctx.z3_ctx.0, p, sort.z3_sort);
            match raw {
                Some(ptr) => Self::wrap(&ctx, ptr),
                None => {
                    eprintln!("   ⚠️  Z3 out of memory (fresh_const allocation failed). Exiting.");
                    std::process::exit(101);
                }
            }
        }
