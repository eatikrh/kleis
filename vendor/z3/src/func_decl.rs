...
impl FuncDecl {
    pub(crate) unsafe fn wrap(ctx: &Context, z3_func_decl: Z3_func_decl) -> Self {
        unsafe {
            let ast = Z3_func_decl_to_ast(ctx.z3_ctx.0, z3_func_decl);
            match ast {
                Some(a) => Z3_inc_ref(ctx.z3_ctx.0, a),
                None => {
                    eprintln!("   ⚠️  Z3 out of memory (FuncDecl ref-count failed). Exiting.");
    ...
    pub fn new<S: Into<Symbol>>(name: S, domain: &[&Sort], range: &Sort) -> Self {
        ...
        unsafe {
            ...
            match raw {
                ...
                None => {
                    eprintln!("   ⚠️  Z3 out of memory (FuncDecl allocation failed). Exiting.");
                    std::process::exit(101);
                }
            }
        }
